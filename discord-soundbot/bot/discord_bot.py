...
# up on a single play attempt. Env-var only (not dashboard-exposed) since
# this is a resilience knob, not something you'd tune day-to-day.
VOICE_CONNECT_MAX_ATTEMPTS = int(os.environ.get("VOICE_CONNECT_MAX_ATTEMPTS", "3"))
VOICE_CONNECT_RETRY_DELAY_SECONDS = float(os.environ.get("VOICE_CONNECT_RETRY_DELAY_SECONDS", "2"))
...
class SoundBot(discord.Client):
    ...
    async def _connect_with_retry(
        ...
        for attempt in range(1, VOICE_CONNECT_MAX_ATTEMPTS + 1):
            try:
                ...
                logger.warning(
                    ...
                    attempt, VOICE_CONNECT_MAX_ATTEMPTS, channel.id, guild.id,
                )
